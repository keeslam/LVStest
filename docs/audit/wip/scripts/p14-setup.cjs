...
(async () => {
  ...
  const mkVehicle = async (plate, brand, model, extra = {}) => {
    ...
    const r = await st.post('/api/vehicles', Object.assign({ licensePlate: plate, brand, model, vehicleType: 'sedan', chassisNumber: 'AUDITP14' + plate.replace(/-/g, ''), fuel: 'Benzine', currentMileage: 12345, productionDate: '2021', notes: 'AUDIT-P14 fixture' }, extra));
    if (r.status !== 201 && r.status !== 200) throw new Error('vehicle ' + plate + ' -> ' + L.short(r));
    return r.json.id;
  };
  const LONG300 = 'AUDIT-P14-Brand-' + 'Ab'.repeat(142); // 300 chars
  const LONG300m = 'AUDIT-P14-Model-' + 'Cd'.repeat(142);
  ids.vNormal = await mkVehicle('AU-140-X', 'AUDIT-P14 Brand', 'AUDIT-P14 Model');
  ids.vLong = await mkVehicle('AU-14A-X', LONG300, LONG300m);
  ids.vSpecial = await mkVehicle('AU-14B-X', 'Citroën', 'C4 Ë-Ürß € 😀 日本語 <b>bold</b>');
  ids.vDamage = await mkVehicle('AU-14C-X', 'AUDIT-P14 DC Brand', 'AUDIT-P14 DC Model', { vehicleType: 'van' });
  ids.vTransport1 = await mkVehicle('AU-144-X', 'AUDIT-P14 TR Brand', 'AUDIT-P14 TR Model');
  ids.vTransport2 = await mkVehicle('AU-145-X', 'AUDIT-P14 TR2 Brand', 'AUDIT-P14 TR2 Model');
  ids.vNoRes = await mkVehicle('AU-146-X', 'AUDIT-P14 NoRes Brand', 'AUDIT-P14 NoRes Model');
  ids.vOldRes = await mkVehicle('AU-147-X', 'AUDIT-P14 OldRes Brand', 'AUDIT-P14 OldRes Model');
  L.log('vehicles', { vNormal: ids.vNormal, vLong: ids.vLong, vSpecial: ids.vSpecial, vDamage: ids.vDamage, vTransport1: ids.vTransport1, vTransport2: ids.vTransport2, vNoRes: ids.vNoRes, vOldRes: ids.vOldRes });

  // ---------- customers ----------
  const mkCustomer = async (data) => {
    const existing = await L.q('select id from customers where name=$1', [data.name]);
    if (existing[0]) return existing[0].id;
  ...
  const NAME500 = 'AUDIT-P14 Long ' + 'Naam '.repeat(97); // 500 chars
  const ADDR1000 = 'AUDIT-P14 Straat ' + '12345 '.repeat(163) + 'x'; // ~1000 chars
  ids.cNormal = await mkCustomer({ name: 'AUDIT-P14 Normal Customer', address: 'Kerkweg 47a', city: 'Zuidland', postalCode: '3214 VC', phone: '0181-451040', driverLicenseNumber: 'NL1234567890', email: 'audit-p14-normal@example.invalid' });
  ids.cLong = await mkCustomer({ name: NAME500.slice(0, 500), address: ADDR1000.slice(0, 1000), city: 'AUDIT-P14 ' + 'Stad'.repeat(60), postalCode: '9999 ZZ', phone: '06' + '1'.repeat(60), driverLicenseNumber: 'L'.repeat(200) });
  ids.cSpecial = await mkCustomer({ name: 'AUDIT-P14 Spéçiål Ünïcödé ß € 😀 日本語 مرحبا <script>alert(1)</script> %s {{customer.name}}', address: 'Straße 1 ½ – “quotes” … → <img src=x onerror=alert(1)>', city: 'Ærø', postalCode: '1234 ÄB', phone: '+31 6 ☎ 12345678', driverLicenseNumber: '%d %s %n {{x}}' });
  ids.cEmpty = await mkCustomer({ name: 'AUDIT-P14 Empty Customer' });
  ids.cFirstLast = await mkCustomer({ name: 'AUDIT-P14 FirstLast Customer', firstName: 'AuditFirst', lastName: 'AuditLast', address: 'Dorpsstraat 1', city: 'Dorp', postalCode: '1111 AA' });
  L.log('customers', { cNormal: ids.cNormal, cLong: ids.cLong, cSpecial: ids.cSpecial, cEmpty: ids.cEmpty, cFirstLast: ids.cFirstLast });
  ...
  await mkRes('rNormal', { vehicleId: ids.vNormal, customerId: ids.cNormal, startDate: D(1), endDate: D(8), totalPrice: 750, notes: 'AUDIT-P14 normal reservation' });
  await mkRes('rLong', { vehicleId: ids.vLong, customerId: ids.cLong, startDate: D(1), endDate: D(8), totalPrice: 1234567.89, notes: 'AUDIT-P14 long ' + 'N'.repeat(20000) });
  await mkRes('rSpecial', { vehicleId: ids.vSpecial, customerId: ids.cSpecial, startDate: D(1), endDate: D(8), totalPrice: 99.5, notes: 'AUDIT-P14 special' });
  await mkRes('rEmpty', { vehicleId: ids.vNormal, customerId: ids.cEmpty, startDate: D(20), endDate: null, notes: 'AUDIT-P14 empty customer, open-ended' });
  await mkRes('rDamage', { vehicleId: ids.vDamage, customerId: ids.cNormal, startDate: D(0), endDate: D(3), totalPrice: 300, notes: 'AUDIT-P14 damage-check reservation' });
  await mkRes('rDamageFL', { vehicleId: ids.vDamage, customerId: ids.cFirstLast, startDate: D(10), endDate: D(12), totalPrice: 200, notes: 'AUDIT-P14 damage-check firstname/lastname' });
  await mkRes('rOld', { vehicleId: ids.vOldRes, customerId: ids.cSpecial, startDate: '2020-01-01', endDate: '2020-01-05', totalPrice: 100, notes: 'AUDIT-P14 old reservation (2020) for damage-check-pdf fallback test' });
  L.log('reservations', { rNormal: ids.rNormal, rLong: ids.rLong, rSpecial: ids.rSpecial, rEmpty: ids.rEmpty, rDamage: ids.rDamage, rDamageFL: ids.rDamageFL, rOld: ids.rOld });
  ...
  await mkTransport('trNormal', { vehicleId: ids.vTransport1, transportType: 'delivery', scheduledDate: D(2), originAddress: 'Kerkweg 47a', originCity: 'Zuidland', destinationAddress: 'Dam 1', destinationCity: 'Amsterdam', distanceKm: 85.5, tollCost: 0, driverName: 'AUDIT-P14 Driver', reason: 'AUDIT-P14 normal transport', notes: 'AUDIT-P14 normal notes', customerId: ids.cNormal, billable: true, billableAmount: 120 });
  await mkTransport('trLong', { vehicleId: ids.vTransport2, transportType: 'tow', scheduledDate: D(3), originAddress: 'AUDIT-P14 ' + 'Lange Straatnaam '.repeat(60), originCity: 'Stad'.repeat(50), destinationAddress: 'B'.repeat(1000), destinationCity: 'C'.repeat(300), driverName: 'D'.repeat(300), reason: 'R'.repeat(2000), notes: 'AUDIT-P14 notes ' + 'lorem ipsum '.repeat(1700), customerId: ids.cLong });
  ...
  await mkTransport('trExternal', { isExternalVehicle: true, externalLicensePlate: 'AU-14E-X', externalBrand: 'AUDIT-P14 Ext Brand', externalModel: 'Ext Model', externalOwnerName: 'AUDIT-P14 Owner', transportType: 'tow', scheduledDate: D(6), originCity: 'A', destinationCity: 'B' });
  L.log('transports', { trNormal: ids.trNormal, trLong: ids.trLong, trSpecial: ids.trSpecial, trEmpty: ids.trEmpty, trExternal: ids.trExternal });

  // ---------- limited user (view_vehicles only) ----------
  if (!ids.limitedUser) {
    const uname = 'AUDIT-P14-limited';
    const ex = await L.q('select id from users where username=$1', [uname]);
    if (ex[0]) { ids.limitedUser = ex[0].id; }
    else {
      const r = await st.post('/api/users', { username: uname, password: 'AuditP14!limited', fullName: 'AUDIT P14 limited', email: 'audit-p14-limited@example.invalid', role: 'user', permissions: ['view_vehicles'], active: true });
      if (r.status !== 201 && r.status !== 200) throw new Error('user -> ' + L.short(r));
      ids.limitedUser = r.json.id;
    }
  }
  ids.limitedUserName = 'AUDIT-P14-limited';
  ids.limitedUserPass = 'AuditP14!limited';
  ...
  await mkPdfTemplate('tplFull', 'AUDIT-P14 full fields', fullFields);
  const invalidFields = [
  ...
  await mkPdfTemplate('tplInvalid', 'AUDIT-P14 invalid fields', invalidFields);
  await mkPdfTemplate('tplEmpty', 'AUDIT-P14 empty fields', []);
  await mkPdfTemplate('tplBg', 'AUDIT-P14 background tests', fullFields);
  L.log('pdf templates', { tplFull: ids.tplFull, tplInvalid: ids.tplInvalid, tplEmpty: ids.tplEmpty, tplBg: ids.tplBg });
  ...
  if (!ids.trTpl) {
    const r = await st.post('/api/transport-report-templates', { name: 'AUDIT-P14 transport template', isDefault: false, fields: trFields });
    if (r.status !== 201) throw new Error('transport template -> ' + L.short(r));
    ids.trTpl = r.json.id;
  }
  ...
  const dcFields = [
    CF('t1', 'text', 40, 120, 'AUDIT-P14 Schadecheck', { fontSize: 16, isBold: true }),
    ...
    CF('p2', 'text', 40, 120, 'AUDIT-P14 pagina 2 tekst', { page: 2 }), CF('p2d', 'dynamic', 40, 150, 'Notities p2', { source: 'notes', page: 2 }),
  ];
  const mkDcTemplate = async (key, body) => {
    if (ids[key]) return ids[key];
    const r = await st.post('/api/damage-check-templates', body);
    if (r.status !== 201) throw new Error('dc template ' + key + ' -> ' + L.short(r));
    ids[key] = r.json.id;
    return r.json.id;
  };
  await mkDcTemplate('dcTpl', { name: 'AUDIT-P14 damage check template', description: 'AUDIT-P14', vehicleMake: 'AUDIT-P14 DC Brand', vehicleModel: 'AUDIT-P14 DC Model', vehicleType: null, isDefault: false, language: 'nl', canvasFields: dcFields, headerText: 'AUDIT-P14 header text', footerText: 'AUDIT-P14 footer text' });
  L.log('templates', { trTpl: ids.trTpl, dcTpl: ids.dcTpl });
